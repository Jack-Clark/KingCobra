...
public abstract class AbstractVerifier {
	...
	// key -> (value -> write id). Lets a version order, which names versions by
	// value, be resolved back to the write (and hence the transaction) that
	// produced it. Only populated for VERSION_ORDER_ON.
	protected Map<Long, Map<Long, Long>> kvToWid;
	...
	public AbstractVerifier() {
		...
		kvToWid = new HashMap<Long, Map<Long, Long>>();
    ...
    int op_counter = 0;
		...
		while(true) {
			...
			case 'C':
				...
				// The prev/next links are always recorded (later passes rely on them);
				// only the CO edge itself is optional, so that session guarantees weaker
				// than serializability can be checked.
				if (prev_txn != null) {
					if (VeriConstants.CLIENT_ORDER_ON) {
			...
			case 'W': {
				...
				if (!kvToWid.containsKey(key_hash)) {
					kvToWid.put(key_hash, new HashMap<Long, Long>());
				}

				// A version order identifies a version by its value, so values must be
				// unique per key for that identification to be well defined. Histories
				// from history_generator.py satisfy this by construction.
				assert !kvToWid.get(key_hash).containsKey(val_hash);
				kvToWid.get(key_hash).put(val_hash, wid);
